...
public:
    ...
    void initTransformMatrices() {
        // 初始化一些常用的变换矩阵
        transformMatrices["rotate90"] = getRotationMatrix2D(Point2f(srcImg.cols/2.0, srcImg.rows/2.0), 90, 1.0);
    }
    
    Mat rotate(double angle) {
        Mat rotated;
        Mat matrix = getRotationMatrix2D(Point2f(srcImg.cols/2.0, srcImg.rows/2.0), angle, 1.0);
